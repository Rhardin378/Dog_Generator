...
fetch(BREEDS_URL)
    ...
    .then(data => {  // Turn the message into an object
    ...
    select.addEventListener('change', event => {
     console.log(event.target.value)
        let url = `https://dog.ceo/api/breed/${event.target.value}/images/random`  // gets the dog breed from the current <option> (the event target is the option and the value is the current option value)
        getDoggoImg(url)
        doggoInfo.assignMF()
        doggoInfo.assignAge()
    ...
    assignName(array) { // using the array parameter so we can pass in male names or female names array  will use this.fNames or this.mNames
        this.rname = array[Math.floor(Math.random() * array.length)]
        document.getElementById("dog-name").innerHTML = `${this.rname}` 
    },
    
    assignAge() {
        this.age = Math.floor(Math.random() * 16 + 1) //pluss one will make sure that the age is not 0
        document.getElementById('age').innerHTML = `Age: ${this.age}`
    },
    
    // randomize the array and take first 2 items out of the array 
